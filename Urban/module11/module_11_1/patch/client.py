...
from .enums import HistoricalKlinesType, ContractType
...
class BaseClient:
    ...
    def _create_futures_api_uri_v2(self, path: str) -> str:
        url = self.FUTURES_URL
        if self.testnet:
            url = self.FUTURES_TESTNET_URL
...
class Client(BaseClient):
    ...
    def __init__(
        ...
        private_key: Optional[Union[str, Path]] = None, private_key_pass: Optional[str] = None, session_params: Optional[Dict[str, str]] = None
    ):
    ...
    def _init_session(self) -> requests.Session:
        ...
        session.headers.update(headers, **self._session_params)
        return session
    ...
    def _request_futures_api_v2(self, method, path, signed=False, **kwargs) -> Dict:
    ...
    def _klines(self, klines_type: HistoricalKlinesType = HistoricalKlinesType.FUTURES_PERP, **params) -> Dict:
        ...
        elif HistoricalKlinesType.FUTURES_PERP == klines_type:
            return self.futures_continous_klines(**params)
        else:
            raise NotImplementedException(klines_type)

    def _get_earliest_valid_timestamp(self, symbol, contract_type, interval, klines_type: HistoricalKlinesType = HistoricalKlinesType.FUTURES_PERP):
        ...
        """
        if HistoricalKlinesType.FUTURES_PERP == klines_type:
            kline = self._klines(
                klines_type=klines_type,
                pair=symbol,
                contractType=contract_type.value.upper(),
                interval=interval
            )
        else:
            kline = self._klines(
    ...
    def get_historical_klines(self, symbol, contract_type: ContractType.PERPETUAL, interval, start_str=None, end_str=None, limit=1000,
                              klines_type: HistoricalKlinesType = HistoricalKlinesType.FUTURES_PERP):
        ...
        return self._historical_klines(
            symbol, contract_type, interval, start_str=start_str, end_str=end_str, limit=limit, klines_type=klines_type
        )

    def _historical_klines(self, symbol, contract_type: ContractType.PERPETUAL, interval, start_str=None, end_str=None, limit=1000,
                           klines_type: HistoricalKlinesType = HistoricalKlinesType.FUTURES_PERP):
        ...
        if start_ts is not None:
            first_valid_ts = self._get_earliest_valid_timestamp(symbol, contract_type, interval, klines_type)
...
class AsyncClient(BaseClient):
    ...
    async def _request_futures_api_v2(self, method, path, signed=False, **kwargs) -> Dict:
        uri = self._create_futures_api_uri_v2(path)
    ...
    async def _klines(self, klines_type: HistoricalKlinesType = HistoricalKlinesType.SPOT, **params) -> Dict:
        ...
        if HistoricalKlinesType.SPOT == klines_type:
            ...
            return await self.futures_continous_klines(**params)
        else:
            raise NotImplementedException(klines_type)
    _klines.__doc__ = Client._klines.__doc__

    async def _get_earliest_valid_timestamp(self, symbol, interval, contract_type: ContractType = ContractType.PERPETUAL,
                                            klines_type: HistoricalKlinesType = HistoricalKlinesType.SPOT):
        if HistoricalKlinesType.FUTURES_PERP == klines_type:
    ...
    async def get_historical_klines(self, symbol, interval, contract_type: ContractType = ContractType.PERPETUAL, start_str=None, end_str=None, limit=1000,
                                    klines_type: HistoricalKlinesType = HistoricalKlinesType.SPOT):
        return await self._historical_klines(symbol, interval, contract_type, start_str, end_str=end_str, limit=limit, klines_type=klines_type)
    get_historical_klines.__doc__ = Client.get_historical_klines.__doc__


    async def _historical_klines(self, symbol, interval, contract_type: ContractType = ContractType.PERPETUAL, start_str=None, end_str=None, limit=1000,
                                 klines_type: HistoricalKlinesType = HistoricalKlinesType.SPOT):
        ...
        if start_ts is not None:
            if HistoricalKlinesType.FUTURES_PERP == klines_type:
                first_valid_ts = await self._get_earliest_valid_timestamp(symbol, interval, contract_type, klines_type)
            else:
    ...
    async def futures_continous_klines_edited(self, symbol, interval, start_str, end_str=None, limit=500):
        return await self._continous_klines(symbol, interval, start_str, end_str=end_str, limit=limit, klines_type=HistoricalKlinesType.FUTURES)
    ...
    async def futures_account_balance_v2(self, **params):
        return await self._request_futures_api_v2('get', 'balance', True, data=params)
    ...
    async def futures_position_information_v2(self, **params):
        return await self._request_futures_api_v2('get', 'positionRisk', True, data=params)
